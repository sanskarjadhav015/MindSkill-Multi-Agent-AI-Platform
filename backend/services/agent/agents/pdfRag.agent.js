...
const LLM_TIMEOUT_MS = Number(process.env.PDFRAG_LLM_TIMEOUT_MS) || 60_000;
...
const cleanupVectorCollection = async (store, collectionName, logCtx) => {
    ...
    try {
        if (typeof store.client?.deleteCollection === "function") {
...
export const pdfRag = async (state) => {
    ...
    try {
        ...
        // Clean up redundant pagination footers generated by pdf-parse (e.g. "-- 1 of 7 --")
        ...
        const docs = await splitter.createDocuments([cleanText || text]);
        ...
        const isBroadQuery = !state.prompt?.trim() || /^(analyse|analyze|summary|summarize|explain|review|overview|what is this|tell me about)/i.test(state.prompt.trim());
        const searchQuery = isBroadQuery ? "Overview, summary, background, skills, experience, and key content of this document." : state.prompt.trim();

        let relevantDocs = [];
        try {
            relevantDocs = await withTimeout(store.similaritySearch(searchQuery, 8), SEARCH_TIMEOUT_MS, "Similarity search");
        } catch (searchError) {
        ...
        const chunkContext = (relevantDocs ?? [])
            .map((d) => d.pageContent.trim())
            .filter((c) => c.length > 30)
            .join("\n\n---\n\n");

        // Build comprehensive context:
        // If document is small/medium (<= 25,000 chars), pass the full clean text.
        // If larger, provide both the top semantic search chunks AND the first 10,000 characters.
        let fullContext = "";
        if (cleanText.length <= 25000) {
            fullContext = cleanText;
        } else {
            fullContext = `=== RELEVANT EXCERPTS ===\n${chunkContext}\n\n=== DOCUMENT BEGINNING ===\n${cleanText.slice(0, 10000)}`;
        }

        // 7. Invoke Grounded LLM
        const llm = await getmodel("pdfRag");

        const userPrompt = state.prompt?.trim() || "Provide a comprehensive analysis and summary of this document.";

        const messages = [
            new SystemMessage(
                `You are MindSkill PDF Assistant.
Rules:
- Analyze the uploaded PDF context provided and fulfill the user's request thoroughly.
- If the user asks general questions like "analyse", "summarize", or "review", provide a comprehensive analysis of the document (sections, key details, takeaways).
- If specific information is requested but completely absent from the document, state: "I couldn't find this information in the uploaded PDF."
